...
# The not-optimised pipeline sets USE_SUITE_TOKEN=true to use the static feature
# suite analytics token (older bktec / collector-gem path doesn't accept OIDC JWTs).
...
if [ "${USE_SUITE_TOKEN}" == "true" ]; then
  echo "Using static FEATURE_SUITE_TOKEN for analytics ingestion"
